...
export default function LegalCookies() {
  return <div className="mmd-public-page mmd-legal-page">
    ...
    <section className="mmd-section"><div className="mmd-container mmd-legal-layout">
      <aside className="mmd-legal-summary"><Cookie/><strong>Choix simple</strong><p>Les cookies techniques indispensables ne peuvent pas être désactivés depuis le site. Les catégories optionnelles restent désactivées tant qu’elles ne sont pas nécessaires et acceptées.</p></aside>
      <div className="mmd-legal-content">
        <LegalSection icon={<ShieldCheck/>} title="1. Cookies strictement nécessaires"><p>Les espaces connectés utilisent les mécanismes de session indispensables à l’authentification et à la sécurité. Ces données techniques permettent notamment de maintenir la connexion et de protéger les accès réservés.</p></LegalSection>
        <LegalSection icon={<Database/>} title="2. Stockage local sur votre appareil"><p>Le site utilise également le stockage local du navigateur pour mémoriser certains choix d’interface. Le formulaire de candidature peut y conserver un <strong>brouillon texte</strong> afin d’éviter de perdre votre saisie avant l’envoi. La photo sélectionnée n’est pas sauvegardée dans ce brouillon local.</p><p>La PWA peut utiliser le cache du navigateur pour permettre un chargement plus rapide et une expérience plus proche d’une application.</p></LegalSection>
        <LegalSection icon={<SlidersHorizontal/>} title="3. Analytics et marketing"><p>Le script Analytics historique a été retiré de la version 2027 tant qu’aucun service d’audience correctement configuré n’est nécessaire. Aucun cookie marketing n’est actuellement requis par le site.</p><p>Si une catégorie optionnelle est activée à l’avenir, cette politique et l’interface de consentement devront être mises à jour avant son utilisation lorsque le consentement est requis.</p></LegalSection>
        <LegalSection icon={<Cookie/>} title="4. Modifier ou effacer vos choix"><p>Vous pouvez effacer les cookies, le stockage local et le cache depuis les réglages de votre navigateur. La bannière du site permet également d’enregistrer vos préférences concernant les catégories optionnelles.</p><p>La suppression des données de session entraîne généralement une déconnexion des espaces privés.</p></LegalSection>
      </div>
    </div></section>
  </div>;
}
