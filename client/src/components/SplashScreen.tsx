      ...
      {showControls && (
        <div className="absolute right-4 top-4 flex items-center gap-2 md:right-7 md:top-7">
          <button
            type="button"
            onClick={toggleSound}
            className="flex h-11 w-11 items-center justify-center rounded-full border border-white/25 bg-black/45 text-white/90 backdrop-blur-xl transition hover:border-[#D4AF37]/70 hover:text-[#F5E6C8]"
          ...
          <button
            type="button"
            onClick={onComplete}
            className="flex h-11 items-center gap-2 rounded-full border border-white/25 bg-black/45 px-4 font-grotesk text-xs uppercase tracking-[0.16em] text-white/90 backdrop-blur-xl transition hover:border-[#D4AF37]/70 hover:text-[#F5E6C8]"
