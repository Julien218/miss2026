...
// Projection volontairement limitée : aucun mot de passe, token, date de naissance,
// adresse IP ou photo n'est transmis au cockpit par cette intégration.
...
export async function readRegistrationPage(cursor: string) {
  if (!process.env.DATABASE_URL) {
    throw Object.assign(new Error("Database unavailable"), { code: "DB_NOT_CONFIGURED" });
  }
  const connection = await mysql.createConnection(process.env.DATABASE_URL);
  try {
    const [rows] = await connection.query<RowDataPacket[]>(
      { sql: REGISTRATIONS_SQL, timeout: 10_000 },
      [cursor]
    );
    const records = rows.slice(0, PAGE_SIZE);
    return {
      records,
      nextCursor: rows.length > PAGE_SIZE ? String(records.at(-1)!.id) : null,
    };
  } finally {
...
export function registrationHandler(readPage = readRegistrationPage) {
  return async (req: Request, res: Response) => {
    ...
    try {
      ...
      const years = page.records
        .map((record) => Number((record as { year?: number }).year ?? 0))
        .filter(Boolean);
      res.json({
        collection: "miss-mister-dour-inscriptions",
        year: years.length ? Math.max(...years) : 2027,
        ...page,
...
export function registerCockpitRegistrationRoutes(app: Express) {
  app.get(
    "/api/integrations/cockpit/registrations",
